declare namespace AMap {
   ...
   *
   * @class VectorLayer
   * @template VectorOverlay extends Overlays
  ...
  class VectorLayer extends Layer<VectorLayer.Options> {
    ...
    public add(vectors: Overlays | Overlays[]): void;
    /** 删除矢量覆盖物 */
    public remove(vectors: Overlays | Overlays[]): void;
    /** 清空矢量覆盖物 */
    public clear(): void;
    /** 判断图层中是否已存在矢量覆盖物 */
    public has(vector: Overlays): boolean;
    /** 根据经纬度查询矢量覆盖物信息 */
    public query(geometry: LngLatLike): Overlays | undefined;
    /** 获取所有覆盖物显示的范围 */
