...
export function useContextMenu (props: UseContextMenu) {
  ...
  useEffect(() => {
    if (AMap && map && !contextMenu) {
      ...
      const rightClick = (event?: AMap.MapsEvent<AMap.MapEventType, unknown>) => {
        if (event?.lnglat) {
          instance.open(map, position || event.lnglat.toArray());
        }
      };
