...
export function useWeather (props: UseWeather) {
    ...
    if (weather && city && type) {
      if (type === "live") {
        weather.getLive(city, (states, result) => {
          if (states === "complete" && onComplete) {
            onComplete(result as AMap.Weather.LiveResult);
          } else if (onError) {
            onError(result as string);
          }
        });
      }
      if (type === "forecast") {
        weather.getForecast(city, (states, result) => {
          if (states === "complete" && onComplete) {
            onComplete(result as AMap.Weather.ForecastResult);
          } else if (onError) {
            onError(result as string);
          }
        });
