...
export function useGeolocation (props: UseGeolocation) {
  ...
  const callback: AMap.Geolocation.Callback = (status, result) => {
    if (status === "complete" && onComplete) {
      onComplete(result as AMap.Geolocation.Result);
    } else if (onError) {
      onError(result as AMap.Geolocation.Error);
    }
  };
  ...
  useEffect(() => {
    if (!/^(position|cityInfo)$/.test(type)) {
      return;
    }
