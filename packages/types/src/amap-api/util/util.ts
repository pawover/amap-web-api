  ...
  namespace AMap {
     ...
     */
      ...
      function rgbHex2Rgba<
        HEX extends string,
        R extends (HEX extends `#${string}` ? never : HEX["length"] extends 6 ? Color : `rgba(${string})`),
      > (hex: HEX): R;
      /**
      ...
      function argbHex2Rgba<
        HEX extends string,
        R extends (HEX extends `#${string}` ? never : HEX["length"] extends 8 ? Color : `rgba(${string})`),
      > (hex: HEX): R;
      /**
       * 判断对象或数组是否为空
       *
       * @template T extends PlainObject | unknown[]
       * @param {T} target 目标对象或数组
       * @returns {boolean} 是否为空
       */
      function isEmpty<T extends PlainObject | unknown[]> (target: T): boolean;
      /**
       * 从目标数组中删除指定项
       *
       * @template T = unknown
       ...
       */
      function requestIdleCallback (callback: AnyFunction, options?: { timeout?: number | undefined }): number;
      /**
      ...
      function parseLngLatData<
        L extends LngLatLike | LngLatLike[],
        R extends (L extends LngLatLike ? LngLat : LngLat[]),
      > (l: L): R;
    }
  }
}
