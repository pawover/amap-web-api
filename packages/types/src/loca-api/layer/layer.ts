...
declare global {
  namespace Loca {
     ...
     * @template {AnyObject} [S=AnyObject]
     * @extends {AMap.Event<"add">} 类 - 地图事件
     */
    abstract class Layer<G extends GeoJSON = GeoJSON, S extends AnyObject = AnyObject> extends AMap.Event<"add"> {
      /**
       * 构造函数
       *
       * @constructor
       ...
       *
       * @public
       * @param {[number,number]} position 像素点位置 ，一般来说是鼠标相对地图容器的像素位置 [x, y]
       */
      public queryFeature (position: [number, number]): Layer.Options | undefined;
      /**
       ...
       *
       * @public
       * @param {Layer.AnimateConfigs} configs 动画配置参数
       * @param {?() => void} [callback] 动画完成后的回调函数
       */
      public addAnimate (configs: Layer.AnimateConfigs, callback?: () => void): void;
      /**
       * 清除图层动画效果，恢复初始状态
       *
       * @public
       ...
       *
       * @public
       * @param {GeoJSONSource<G>} data 数据源
       * @param {?S} [options] 渲染样式
       */
      public setSource (data: GeoJSONSource<G>, options?: S): void;
      /**
       ...
       * @param {S} options 渲染样式
       */
      public setStyle (options: S): void;
      /** 获取图层标注 */
