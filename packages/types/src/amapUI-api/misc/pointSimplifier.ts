  ...
  namespace AMapUI {
    namespace PointSimplifier {
      interface Options<D = any> {
        ...
        renderConstructor?:
          typeof PointSimplifier.Render.Canvas | typeof PointSimplifier.Render.Canvas.GroupStyleRender;
        /** 绘制引擎的构造参数 */
        renderOptions?: RenderOptions<D>;
        /**
        ...
        /**
         * 返回数据项中的经纬度信息
         * @param  {D} dataItem 数据项
         * @param  {number} dataIndex 数据项索引
         * @return {string | null} 经纬度信息
         */
        getPosition: (dataItem: D, dataIndex: number) => AMap.LngLatLike;
        /**
         * 返回鼠标悬停时的显示内容
         * @param  {D} dataItem 数据项
         * @param  {number} dataIndex 数据项索引
         ...
         */
        getHoverTitle?: (dataItem: D, dataIndex: number) => string | null;
        /**
         * 数据项的优先级比较函数
         * - 用于排序，排序后的数组，靠前的优先绘制
         * @param  {D} a 数据项 a
         ...
         * @return {number} `-1` `0` `1`
         */
        compareDataItem?: (a: D, b: D, aIndex: number, bIndex: number) => -1 | 0 | 1;
      }
      /**
       * 绘制引擎的构造参数
       */
      interface RenderOptions<D = any> extends GroupStyleRender<D> {
        /**
         * 是否开启事件监听
      ...
      interface GroupStyleRender<D = any> {
        /**
         * 获取某个数据项的分组 Id
         ...
         */
        getGroupId?: (dataItem: D, dataIndex: number) => unknown;
        /** 设置或获取样式配置 */
        groupStyleOptions?:
          Record<PropertyKey, GroupPointStyle | undefined> | ((id: unknown) => GroupPointStyle | undefined);
      }
       ...
       * 事件绑定回调函数
       */
      type PointEventCallback<T, D> = (
        e: { type: T; target: AMap.Map; originalEvent: AMap.MapsEvent<"click" | "mousemove", AMap.Map> },
        info: { index: number; data: D },
      ) => void;
    }
     ...
     * @template [D=any] 单个数据模型，自身的格式没有强制要求，只要能索引或者包含轨迹的相关信息即可
     */
    class PointSimplifier<D = any> {
       ...
       * @constructor
       * @public
       * @param {PointSimplifier.Options<D>} options 构造参数
       */
      public constructor (options?: PointSimplifier.Options<D>);
       ...
       * @param {D[]} data 数据源
       */
      public _buildData (data: D[]): void;
      /** 渲染数据准备完毕，开始渲染前触发 */
      public onRenderReady (callback: AnyFunction): void;
      /** 判断图层是否处于隐藏状态 */
      ...
      /** 获取图层的构造参数 */
      public getOptions (): PointSimplifier.Options<D>;
      /** 获取绘制引擎 */
      public getRender (): CanvasRenderingContext2D;
      /** 获取绘制引擎的构造参数 */
      public getRenderOptions (): PointSimplifier.RenderOptions<D>;
      /** 获取数据总数 */
      public getTotalPointsNum (): number;
      /** 根据索引获取数据项 */
      public getDataItemByIndex (index: number): D | undefined;
      /** 获取单一事件的监听数量 */
      public listenerLength (eventName: PointEventType): number;
       ...
       */
      public on<T extends PointEventType> (
        eventName: T | `${T} ${T}` | `${T} ${T} ${T}`,
        callback: PointSimplifier.PointEventCallback<T, D>,
      ): void;
      /**
       * 解除事件绑定
       * - 多个事件用空格分隔
