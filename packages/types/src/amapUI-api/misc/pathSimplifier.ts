  ...
  namespace AMapUI {
    namespace PathSimplifier {
      interface Options<D = any> {
         ...
         * 返回轨迹数据中的路径信息
         *
         * @param  {D} pathData 轨迹数据项
         * @param  {number} pathIndex 轨迹索引
         * @return {AMap.LngLatLike[]} 轨迹的节点坐标数组
         */
        getPath: (pathData: D, pathIndex: number) => AMap.LngLatLike[];
        /**
         ...
         */
        getHoverTitle?: (pathData: D, pathIndex: number, pointIndex: number) => string | null;
      }
      /**
       * 绘制引擎的构造参数
       */
      interface RenderOptions<D = any> extends PlainObject {
        /**
         * 是否开启事件监听
         ...
         * @param  {D} pathItem.pathData 轨迹数据
         * @param  {number} zoom 当前地图的缩放级别
         * @return {RenderOptions<D>} 样式配置
         */
        getPathStyle?: (pathInfo: { pathIndex: number; pathData: D }, zoom: number) => Partial<RenderOptions<D>>;
      }
      /**
       * 轨迹节点的绘制样式
       */
       ...
       * 事件绑定回调函数
       */
      type PathEventCallback<T, D> = (
        e: { type: T; target: AMap.Map; originalEvent: AMap.MapsEvent<"click" | "mousemove", AMap.Map> },
        info: { type: "path" | "point"; pointIndex?: number; pathIndex: number; pathData: D },
      ) => void;
    }
    ...
    class PathSimplifier<D = any> {
       ...
       * @constructor
       * @public
       * @param {PathSimplifier.Options<D>} options 构造参数
       */
      public constructor (options?: PathSimplifier.Options<D>);

      /** 构造参数 */
      public _opts: PathSimplifier.Options<D>;
      ...
      /** 获取图层的构造参数 */
      public getOptions (): PathSimplifier.Options<D>;
      /** 获取绘制引擎 */
      public getRender (): CanvasRenderingContext2D;
      /** 获取绘制引擎的构造参数 */
      public getRenderOptions (): PathSimplifier.RenderOptions<D>;
      /** 获取数据总数 */
      ...
      /** 获取轨迹巡航器列表 */
      public getPathNavigators (): PathNavigator<D>[];
       ...
       * @returns {PathNavigator<D>} 轨迹巡航器
       */
      public createPathNavigator (pathIndex: number, options: PathNavigator.Options): PathNavigator<D>;
      /** 销毁所有轨迹巡航器 */
       ...
       * - 无选中时返回 `null`
       */
      public getSelectedPathData (): D | null;
      /**
       * 获取处于选中状态的轨迹索引
       * - 无选中时返回 `-1`
       */
       ...
       */
      public on<T extends PathEventType> (
        eventName: T | `${T} ${T}` | `${T} ${T} ${T}` | `${T} ${T} ${T} ${T}`,
        // | `${T} ${T} ${T} ${T} ${T}`
        // | `${T} ${T} ${T} ${T} ${T} ${T}`,
        callback: PathSimplifier.PathEventCallback<T, D>,
      ): void;
      /**
       * 解除事件绑定
       * - 多个事件用空格分隔
