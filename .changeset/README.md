...
flowchart TD
    ...
    subgraph S2["阶段二 · alpha 通道（全自动，到 alpha 为止）"]
        ...
        pub_job --> pub_subs["changesets/action/publish：拓扑序发布 2 子包<br/>types → react<br/>自动打 git tag（amap-web-types@1.0.0-beta.5-alpha.0 等）并推送"]
        pub_subs --> pub_root["根包：pnpm build + pnpm publish（幂等）<br/>npm view 已存在则跳过<br/>feature 通道 --tag alpha 发 alpha tag"]
        pub_root --> alpha_ok["✅ npm：amap-web-react@1.0.0-beta.5-alpha.0 等<br/>dist-tag: alpha"]
    end
    ...
    subgraph S3["阶段三 · 发布合并（人工闸门）"]
        ...
        rm4 --> rm5["⑤ 防撞车校验：npm view 检查 2 包剥离后版本"]
